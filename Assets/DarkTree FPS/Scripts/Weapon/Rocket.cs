...
namespace DarkTreeFPS
{
    public class Rocket : MonoBehaviour
    {
        ...
        void Explosion(RaycastHit hit, bool useHit)
        {
            ...
            List<EnemyHealth> enemy = new List<EnemyHealth>();
            foreach (Collider collider in colliders)
            {
                if (hit.collider.GetComponent<BodyPart>()){ 
                    if(!enemy.Contains(hit.collider.GetComponent<BodyPart>().totalHealth)){
                        hit.collider.GetComponent<BodyPart>().ApplyDamage((int)damage);
                        enemy.Add(hit.collider.GetComponent<BodyPart>().totalHealth);
                    }
