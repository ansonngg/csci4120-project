...
namespace DarkTreeFPS {
    ...
    public class Grenade : MonoBehaviour
    {
        ...
        void Explosion()
        {
            ...
            List<EnemyHealth> enemy2 = new List<EnemyHealth>();
            foreach (Collider collider in colliders)
            {
                if (collider.GetComponent<BodyPart>()){ 
                    if(!enemy2.Contains(collider.GetComponent<BodyPart>().totalHealth)){
                        print("collider.GetComponent<BodyPart>() = "+collider.GetComponent<BodyPart>());
                        collider.GetComponent<BodyPart>().ApplyDamage((int)damage);
                        enemy2.Add(collider.GetComponent<BodyPart>().totalHealth);
                    }
